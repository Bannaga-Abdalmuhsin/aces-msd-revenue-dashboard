...
export function formatSAR(value: number | null | undefined): string {
  if (value === null || value === undefined || Number.isNaN(value)) return 'SAR 0';
  const sign = value < 0 ? '-' : '';
...
export function formatSARExact(value: number | null | undefined): string {
  if (value === null || value === undefined || Number.isNaN(value)) return 'SAR 0.00';
...
export function formatMonthFull(value: string | null | undefined): string {
  if (!value) return '—';
  const parts = value.split('-');
  if (parts.length < 2) return value;
  const d = new Date(Number(parts[0]), Number(parts[1]) - 1, 1);
