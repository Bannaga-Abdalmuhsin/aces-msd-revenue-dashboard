...
export function InsightsPanel() {
    ...
    return (<div className="rounded-lg border border-card-border bg-card p-4" data-testid="panel-insights">
      ...
      {!isLoading && !isError && (!data || data.length === 0) && (<EmptyState title="No insights available" description="Insights will appear once revenue data is present."/>)}

      {!isLoading && !isError && data && data.length > 0 && (<div className="space-y-2">
          {data.map((insight, i) => {
                const Icon = SEVERITY_ICON[insight.severity] ?? Info;
                return (<motion.div key={`${insight.type}-${i}`} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25, delay: i * 0.03 }} className="flex items-start gap-3 rounded-md border border-border bg-background/60 p-3" data-testid={`insight-${i}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    {insight.projectName && (<span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                        {insight.projectName}
                      </span>)}
                    {insight.amount !== null && insight.amount !== undefined && (<span className="font-mono-num text-[10px] font-semibold text-foreground">
