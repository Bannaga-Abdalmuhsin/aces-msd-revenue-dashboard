...
interface KpiCardProps {
  ...
  accent?: 'primary' | 'accent' | 'destructive' | 'chart-3';
  index?: number;
  testId?: string;
}
...
export function KpiCard({
  ...
  const content = (
    <motion.div
      ...
      className={cn(
        'group relative flex flex-col justify-between overflow-hidden rounded-lg border border-card-border bg-card p-4 transition-all',
        href && 'cursor-pointer hover:-translate-y-0.5 hover:shadow-md',
      )}
      data-testid={testId ?? `card-kpi-${label.toLowerCase().replace(/\s+/g, '-')}`}
    >
      ...
      <div className="mt-3">
        {exactValue !== undefined ? (
          ...
          <p className="font-mono-num font-display text-xl font-semibold tracking-tight text-foreground">{value}</p>
        )}
        {sublabel && <p className="mt-1 text-xs text-muted-foreground">{sublabel}</p>}
        {trend && (
          <p className={cn('mt-1 text-xs font-medium', trend.positive ? 'text-accent' : 'text-destructive')}>
      ...
      <div
        className={cn(
          'pointer-events-none absolute -right-4 -top-4 h-16 w-16 rounded-full opacity-0 blur-2xl transition-opacity group-hover:opacity-100',
          accent === 'primary' && 'bg-primary/20',
          accent === 'accent' && 'bg-accent/20',
          accent === 'destructive' && 'bg-destructive/20',
          accent === 'chart-3' && 'bg-[hsl(var(--chart-3)/0.2)]',
        )}
      />
    </motion.div>
  );
