...
const buttonVariants = cva('inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0' +
    ...
    variants: {
        variant: {
            default: 
            // Use the shared elevation behavior and primary border.
            'bg-primary text-primary-foreground border border-primary-border',
            destructive: 'bg-destructive text-destructive-foreground shadow-sm border-destructive-border',
            outline: 
            // Show the background color of the containing card, sidebar, or accent surface.
            // Inherits the current text color. Uses shadow-xs. no shadow on active
            // No hover state
            ' border [border-color:var(--button-outline)] shadow-xs active:shadow-none ',
            secondary: 
            // Secondary border without an additional shadow.
            'border bg-secondary text-secondary-foreground border border-secondary-border ',
            // Transparent border preserves button dimensions.
            ghost: 'border border-transparent',
            link: 'text-primary underline-offset-4 hover:underline',
        },
        size: {
            // Dashboard button sizes.
