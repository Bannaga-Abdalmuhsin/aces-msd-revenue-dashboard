...
import { UploadCloud, FileSpreadsheet, FileDown, Printer, Trash2, CheckCircle2, AlertTriangle, X, } from 'lucide-react';
import { useQueryClient } from '@tanstack/react-query';
import { useImportRecords, useClearDemoData, useListRecords, getListRecordsQueryKey, getGetDashboardSummaryQueryKey, getGetMonthlyTrendQueryKey, getGetProjectPerformanceQueryKey, getListProjectsQueryKey, getListInvoicesQueryKey, } from '@workspace/api-client-react';
...
const REQUIRED_FIELDS = ['projectName', 'revenueMonth', 'workOrder', 'revenue'];
function normalizeDate(value) {
    ...
    if (mdy)
        return `${mdy[2]}-${String(mdy[1]).padStart(2, '0')}-01`;
...
function parseRows(raw) {
    return raw.map((row, i) => {
        ...
        if (!projectName)
            warnings.push('Missing project name');
        if (!revenueMonth)
            warnings.push('Missing or invalid revenue month');
        if (workOrder === 0)
            warnings.push('Work order is zero or missing');
        if (revenue === 0)
            warnings.push('Revenue is zero or missing');
        const parsed = {
            _rowIndex: i + 2,
            _warnings: warnings,
            projectName,
            ...
            invoiceNo: get('invoiceNo', 'Invoice No', 'Invoice Number') ?? null,
...
export default function ImportPage() {
    ...
    const duplicateInvoiceNos = useMemo(() => {
        ...
        for (const r of parsedRows) {
            if (r.invoiceNo)
                seen.set(r.invoiceNo, (seen.get(r.invoiceNo) ?? 0) + 1);
        }
        return new Set(Array.from(seen.entries()).filter(([, count]) => count > 1).map(([no]) => no));
    }, [parsedRows]);
    function handleImport() {
        const records = parsedRows.map(({ _rowIndex, _warnings, ...rest }) => rest);
        importMutation.mutate({ data: { records, allowDuplicateInvoices: allowDuplicates } }, {
            onSuccess: (result) => {
    ...
    return (<AppLayout title="Import & Export" description="Client-side Excel/CSV import with validation preview, plus data export">
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <div className="xl:col-span-2 space-y-4">
          <div onDragOver={(e) => {
            e.preventDefault();
            setDragActive(true);
        }} onDragLeave={() => setDragActive(false)} onDrop={onDrop} onClick={() => fileInputRef.current?.click()} className={cn('flex cursor-pointer flex-col items-center justify-center gap-3 rounded-lg border-2 border-dashed p-10 text-center transition-colors', dragActive ? 'border-primary bg-primary/5' : 'border-border bg-card hover:bg-muted/30')} data-testid="dropzone-import">
          ...
          {parsedRows.length > 0 && (<div className="rounded-lg border border-card-border bg-card p-4">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <h3 className="font-display text-sm font-semibold text-foreground">
                  Import Preview — {parsedRows.length} records
                </h3>
                <div className="flex items-center gap-2">
                  {warningCount > 0 && (<span className="flex items-center gap-1 rounded-full border border-[hsl(var(--chart-1)/0.3)] bg-[hsl(var(--chart-1)/0.1)] px-2 py-0.5 text-[11px] font-medium text-[hsl(var(--chart-1))]">
                      <AlertTriangle className="h-3 w-3"/>
                      {warningCount} rows with warnings
                    </span>)}
                  {duplicateInvoiceNos.size > 0 && (<span className="flex items-center gap-1 rounded-full border border-destructive/25 bg-destructive/10 px-2 py-0.5 text-[11px] font-medium text-destructive">
              ...
              <div className="max-h-96 overflow-auto rounded-md border border-border">
                <table className="w-full min-w-[900px] text-left text-[11px]" data-testid="table-import-preview">
                  ...
                  <tbody>
                    {parsedRows.map((r) => (<tr key={r._rowIndex} className={cn('border-b border-border/50 last:border-0', r._warnings.length > 0 && 'bg-[hsl(var(--chart-1)/0.06)]')} data-testid={`row-preview-${r._rowIndex}`}>
                        ...
                        <td className="px-2 py-1.5">
                          {r._warnings.length === 0 ? (<CheckCircle2 className="h-3.5 w-3.5 text-accent"/>) : (<span className="text-[10px] text-[hsl(var(--chart-1))]">{r._warnings.join('; ')}</span>)}
                        </td>
                      </tr>))}
                  </tbody>
                </table>
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <Switch checked={allowDuplicates} onCheckedChange={setAllowDuplicates} data-testid="switch-allow-duplicates"/>
          ...
          {parsedRows.length === 0 && (<EmptyState icon={FileSpreadsheet} title="No file loaded yet" description="Upload an Excel or CSV file above to preview and validate records before importing."/>)}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-destructive/25 bg-destructive/5 p-4">
            ...
            <Button variant="destructive" className="w-full justify-start" onClick={handleClearDemo} disabled={clearDemoMutation.isPending} data-testid="button-clear-demo">
