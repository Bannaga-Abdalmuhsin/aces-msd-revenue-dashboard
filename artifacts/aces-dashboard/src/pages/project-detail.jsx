...
import { LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip as RTooltip, Legend, ResponsiveContainer, } from 'recharts';
...
export default function ProjectDetailPage() {
    ...
    const trendData = useMemo(() => (data?.monthlyTrend ?? []).map((p) => ({ ...p, monthLabel: formatMonthLabel(p.month) })), [data?.monthlyTrend]);
    const invoicedRecords = useMemo(() => (data?.records ?? []).filter((r) => r.invoiceNo), [data?.records]);
    const deductiblesTotal = useMemo(() => (data?.records ?? []).reduce((sum, r) => sum + (r.deductible || 0), 0), [data?.records]);
    const penaltiesTotal = useMemo(() => (data?.records ?? []).reduce((sum, r) => sum + (r.penalties || 0), 0), [data?.records]);
    const monthlyDeductibles = useMemo(() => (data?.monthlyTrend ?? []).map((m, i) => {
    ...
    return (<AppLayout title={data?.project.name ?? 'Project Detail'} description="Monthly performance, invoicing, and deductibles breakdown" actions={<Button variant="outline" size="sm" onClick={() => setLocation('/projects')} data-testid="button-back-projects">
          <ArrowLeft className="mr-1.5 h-3.5 w-3.5"/>
          Back
        </Button>}>
      {isError ? (<ErrorState onRetry={() => refetch()}/>) : isLoading ? (<div className="space-y-4">
          <CardGridSkeleton count={4}/>
          <ChartSkeleton />
        </div>) : !data ? (<EmptyState title="Project not found" description="This project may have been removed."/>) : (<>
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
            <ChartCard title="Monthly Trend" description="Work order, revenue, invoiced, collected over time" index={0}>
              {trendData.length === 0 ? (<div className="flex h-[280px] items-center justify-center text-sm text-muted-foreground">No monthly data</div>) : (<ResponsiveContainer width="100%" height={280}>
            ...
            <ChartCard title="Deductibles & Penalties by Month" description={`Total deductibles ${formatSAR(deductiblesTotal)} · penalties ${formatSAR(penaltiesTotal)}`} index={1}>
              {monthlyDeductibles.length === 0 ? (<div className="flex h-[280px] items-center justify-center text-sm text-muted-foreground">No data</div>) : (<ResponsiveContainer width="100%" height={280}>
          ...
          <div className="mt-4 rounded-lg border border-card-border bg-card p-4">
            ...
            {invoicedRecords.length === 0 ? (<EmptyState title="No invoices yet" description="No invoices have been raised for this project."/>) : (<div className="overflow-x-auto">
                <table className="w-full min-w-[900px] text-left text-xs" data-testid="table-project-invoices">
                  ...
                  <tbody>
                    {invoicedRecords.map((r) => (<tr key={r.id} className="border-b border-border/60 last:border-0" data-testid={`row-invoice-${r.id}`}>
