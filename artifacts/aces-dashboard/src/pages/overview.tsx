...
import {
  ...
  useGetPaymentStatusDistribution,
  useGetAgingReport,
  getGetDashboardSummaryQueryKey,
  getGetMonthlyTrendQueryKey,
  getGetProjectPerformanceQueryKey,
  getGetPaymentStatusDistributionQueryKey,
...
export default function OverviewPage() {
  ...
  const kpis = summary
    ? [
        { label: 'Work Order', value: formatSAR(summary.totalWorkOrder), exact: summary.totalWorkOrder, icon: Briefcase, href: '/projects', accent: 'primary' as const },
        { label: 'Revenue', value: formatSAR(summary.totalRevenue), exact: summary.totalRevenue, icon: TrendingUp, href: '/revenue', accent: 'primary' as const },
        { label: 'Invoiced', value: formatSAR(summary.totalInvoiced), exact: summary.totalInvoiced, icon: FileText, href: '/invoices', accent: 'chart-3' as const },
        { label: 'Collected', value: formatSAR(summary.totalCollected), exact: summary.totalCollected, icon: Wallet, href: '/invoices', accent: 'accent' as const },
        { label: 'Outstanding', value: formatSAR(summary.totalOutstanding), exact: summary.totalOutstanding, icon: CircleDollarSign, href: '/aging', accent: 'chart-3' as const },
        { label: 'Overdue', value: formatSAR(summary.totalOverdue), exact: summary.totalOverdue, icon: AlertTriangle, href: '/aging', accent: 'destructive' as const },
        { label: 'Deductibles', value: formatSAR(summary.totalDeductible), exact: summary.totalDeductible, icon: ShieldMinus, href: '/deductibles', accent: 'destructive' as const },
        { label: 'Penalties', value: formatSAR(summary.totalPenalties), exact: summary.totalPenalties, icon: Gavel, href: '/deductibles', accent: 'destructive' as const },
        { label: 'Net Revenue', value: formatSAR(summary.totalNetRevenue), exact: summary.totalNetRevenue, icon: PiggyBank, href: '/revenue', accent: 'accent' as const },
        { label: 'Collection Rate', value: formatPercent(summary.collectionRate), icon: Percent, href: '/invoices', accent: 'accent' as const },
  ...
  return (
    <AppLayout
      ...
      description="Consolidated financial command center across all managed services projects"
      lastDataUpdate={summary?.lastDataUpdate}
    >
      <FilterBar />
      ...
      {summaryQ.isError ? (
        ...
        <CardGridSkeleton count={13} />
      ) : (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5" data-testid="grid-kpi-cards">
          {kpis.map((kpi, i) => (
            <KpiCard
              key={kpi.label}
              label={kpi.label}
              value={kpi.value}
              exactValue={kpi.exact}
              icon={kpi.icon}
              href={kpi.href}
              accent={kpi.accent}
              index={i}
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <ChartCard title="Project Performance" description="Revenue vs work order by project" index={1}>
          {perfQ.isLoading ? (
            ...
            <div className="flex h-[280px] items-center justify-center text-sm text-muted-foreground">No project data</div>
          ) : (
        ...
        <ChartCard title="Payment Status Distribution" description="Invoice count by payment status" index={2}>
          {statusQ.isLoading ? (
            ...
            <div className="flex h-[280px] items-center justify-center text-sm text-muted-foreground">No invoice data</div>
          ) : (
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie
                ...
                <RTooltip
                  formatter={(v: number, n, p: any) => [`${v} invoices — ${formatSAR(p.payload.amount)}`, p.payload.status]}
                  contentStyle={{ background: 'hsl(var(--popover))', border: '1px solid hsl(var(--border))', borderRadius: 8, fontSize: 12 }}
                />
                <Legend wrapperStyle={{ fontSize: 11 }} />
              </PieChart>
      ...
      <div className="mt-4">
        <ChartCard title="Outstanding Aging Summary" description="Distribution of outstanding amounts by age bucket" index={3}>
          {agingQ.isLoading ? (
            ...
            <ErrorState onRetry={() => agingQ.refetch()} />
          ) : !agingQ.data || agingQ.data.buckets.length === 0 ? (
            <div className="flex h-[220px] items-center justify-center text-sm text-muted-foreground">No aging data</div>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={agingQ.data.buckets}>
                ...
                <YAxis tickFormatter={(v) => formatSAR(v)} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} width={70} />
                <RTooltip
                  formatter={(v: number, n, p: any) => [formatSAR(v), `${p.payload.count} invoices`]}
                  contentStyle={{ background: 'hsl(var(--popover))', border: '1px solid hsl(var(--border))', borderRadius: 8, fontSize: 12 }}
                />
                <Bar dataKey="amount" name="Outstanding Amount" radius={[4, 4, 0, 0]}>
                  {agingQ.data.buckets.map((b, i) => (
                    <Cell
                      key={b.label}
                      fill={
                        b.label === 'Not Due'
