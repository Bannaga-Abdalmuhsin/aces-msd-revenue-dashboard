...
export default function ForecastPage() {
  ...
  const chartData = useMemo(
    () => (data?.months ?? []).map((m) => ({ ...m, monthLabel: formatMonthLabel(m.month) })),
    [data?.months],
  );

  return (
    <AppLayout title="Monthly Forecast" description="Expected collections and revenue projections by due date month">
      {isLoading ? (
        ...
        <ErrorState title="No forecast data" description="Forecast data is unavailable." />
      ) : (
        <>
          ...
          <div className="mt-4">
            <ChartCard title="Work Order vs Actual / Forecast Revenue" description="Historical actuals shown solid, projected months shown as forecast" index={3}>
              {chartData.length === 0 ? (
                <div className="flex h-[340px] items-center justify-center text-sm text-muted-foreground">No forecast data</div>
              ) : (
                <ResponsiveContainer width="100%" height={340}>
                  <ComposedChart data={chartData}>
                    ...
                    <RTooltip
                      formatter={(v: number, name: string, p: any) => [formatSAR(v), `${name} (${p.payload.isActual ? 'Actual' : 'Forecast'})`]}
                    ...
                    <Bar dataKey="expectedCollection" name="Expected Collection" radius={[3, 3, 0, 0]}>
                      {chartData.map((entry) => (
                        <Cell key={entry.month} fill={entry.isActual ? 'hsl(var(--chart-2))' : 'hsl(var(--chart-1) / 0.55)'} />
                      ))}
                    </Bar>
          ...
          <div className="mt-4 rounded-lg border border-card-border bg-card p-4">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px] text-left text-xs" data-testid="table-forecast">
                ...
                <tbody>
                  {chartData.map((m) => (
                    <tr key={m.month} className="border-b border-border/60 last:border-0" data-testid={`row-forecast-${m.month}`}>
