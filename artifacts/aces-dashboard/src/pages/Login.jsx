...
export default function Login({ onLogin }) {
    ...
    return (<div className="min-h-screen grid lg:grid-cols-[1.08fr_0.92fr]" style={{ fontFamily: 'Inter, sans-serif' }}>
      <section className="relative overflow-hidden px-7 py-7 sm:px-12 sm:py-10 lg:px-16 lg:py-12 flex flex-col justify-between min-h-[280px] lg:min-h-screen" style={{ background: 'linear-gradient(145deg, #081B3A 0%, #112E63 58%, #0A224A 100%)' }}>
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          <div className="w-36 h-16 sm:w-40 sm:h-[72px] overflow-hidden flex items-center justify-center">
            <img src={logoUrl} alt="ACES Managed Services Logo" className="w-full h-auto" style={{ mixBlendMode: 'screen', transform: 'scale(1.18)', transformOrigin: 'center' }}/>
          </div>
        ...
        <div className="relative z-10 hidden lg:flex items-center gap-3 flex-wrap">
          {['Revenue', 'Invoicing', 'Collections'].map((label, index) => (<div key={label} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border text-xs font-semibold text-white" style={{ borderColor: 'rgba(117,161,215,0.5)', background: 'rgba(5,21,49,0.3)' }}>
      ...
      <main className="relative flex items-center justify-center px-5 py-10 sm:px-10" style={{ background: '#F8FAFC' }}>
        ...
        <div className="w-full max-w-[500px] bg-white rounded-2xl px-6 py-8 sm:px-10 sm:py-10" style={{ border: '1px solid #E2E8F0', boxShadow: '0 20px 55px rgba(17,35,72,0.10)' }}>
          <div className="mb-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" autoComplete="username" value={username} onChange={e => setUsername(e.target.value)} required placeholder="Enter your username" className="w-full text-sm pl-12 pr-4 py-3.5 rounded-xl border outline-none transition-all bg-white" style={{ borderColor: '#CBD5E1', color: '#1A202C' }} onFocus={fieldFocus} onBlur={fieldBlur}/>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPw ? 'text' : 'password'} autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} required placeholder="Enter your password" className="w-full text-sm pl-12 pr-16 py-3.5 rounded-xl border outline-none transition-all bg-white" style={{ borderColor: '#CBD5E1', color: '#1A202C' }} onFocus={fieldFocus} onBlur={fieldBlur}/>
                <button type="button" onClick={() => setShowPw(v => !v)} tabIndex={-1} className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1.5 text-xs font-medium" style={{ color: '#64748B' }} aria-label={showPw ? 'Hide password' : 'Show password'}>
            ...
            <button type="submit" disabled={loading} className="w-full py-3.5 rounded-xl text-sm font-bold text-white transition-all disabled:opacity-60 disabled:cursor-not-allowed" style={{ background: '#D9232A', boxShadow: '0 8px 18px rgba(217,35,42,0.20)' }} onMouseOver={e => { if (!loading)
        e.currentTarget.style.background = '#C8102E'; }} onMouseOut={e => { e.currentTarget.style.background = '#D9232A'; }}>
