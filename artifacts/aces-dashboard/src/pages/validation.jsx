...
export default function ValidationPage() {
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter((r) => r.projectName?.toLowerCase().includes(q) ||
    ...
    return (<AppLayout title="Data Validation" description="Automated data quality checks across missing fields, duplicates, and calculation mismatches">
      ...
      <div className="mb-4 relative max-w-sm">
        ...
        <Input placeholder="Search by project, invoice, or field..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 text-xs" data-testid="input-search-validation"/>
      </div>

      {isError ? (<ErrorState onRetry={() => refetch()}/>) : isLoading ? (<TableSkeleton rows={8} cols={7}/>) : filtered.length === 0 ? (<EmptyState icon={ClipboardCheck} title="No data quality issues" description="All revenue records passed validation checks."/>) : (<div className="overflow-x-auto rounded-lg border border-card-border bg-card">
          <table className="w-full min-w-[1100px] text-left text-xs" data-testid="table-validation-issues">
            ...
            <tbody>
              {filtered.map((issue, i) => (<tr key={`${issue.recordId}-${issue.field}-${i}`} className="border-b border-border/60 last:border-0" data-testid={`row-validation-${issue.recordId}-${i}`}>
