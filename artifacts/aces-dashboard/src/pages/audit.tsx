...
export default function AuditPage() {
  ...
  return (
    <AppLayout title="Administration" description="Full audit trail of data changes across the system">
      {isError ? (
        <ErrorState onRetry={() => refetch()} />
      ) : isLoading ? (
        <TableSkeleton rows={10} cols={6} />
      ) : !data || data.data.length === 0 ? (
        <EmptyState icon={History} title="No audit history" description="Changes to records will be logged here." />
      ) : (
        <>
          <div className="overflow-x-auto rounded-lg border border-card-border bg-card">
            <table className="w-full min-w-[1000px] text-left text-xs" data-testid="table-audit-log">
              ...
              <tbody>
                {data.data.map((log) => (
                  <tr key={log.id} className="border-b border-border/60 last:border-0" data-testid={`row-audit-${log.id}`}>
