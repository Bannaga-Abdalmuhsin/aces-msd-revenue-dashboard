...
export default function AgingPage() {
  ...
  const overdueRows = useMemo(
    () => clientFilterByProjects(invoicesQ.data?.data ?? [], filters),
    [invoicesQ.data?.data, filters],
  );

  function bucketColor(label: string) {
    if (label === 'Not Due') return 'hsl(var(--chart-2))';
    if (label === '90d+') return 'hsl(var(--destructive))';
    if (label === '61-90d') return 'hsl(var(--chart-4))';
    return 'hsl(var(--chart-1))';
  }

  return (
    <AppLayout title="Outstanding Aging" description="Aging buckets and overdue invoice management">
      <FilterBar />

      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <ChartCard
          title="Aging Buckets"
          description={agingQ.data ? `Total outstanding ${formatSAR(agingQ.data.totalOutstanding)}` : undefined}
          index={0}
        >
          {agingQ.isLoading ? (
            <ChartSkeleton />
            ...
            <div className="flex h-[300px] items-center justify-center text-sm text-muted-foreground">No aging data</div>
          ) : (
        ...
        <ChartCard title="Collection Trend" description="Monthly collected amounts" index={1}>
          {trendQ.isLoading ? (
            ...
            <div className="flex h-[300px] items-center justify-center text-sm text-muted-foreground">No collection data</div>
          ) : (
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={trendData}>
                ...
                <YAxis tickFormatter={(v) => formatSAR(v)} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} width={70} />
                <RTooltip
                  formatter={(v: number, n, p: any) => [formatSAR(v), `${p.payload.count} collections`]}
                  contentStyle={{ background: 'hsl(var(--popover))', border: '1px solid hsl(var(--border))', borderRadius: 8, fontSize: 12 }}
                />
      ...
      <div className="mt-4 rounded-lg border border-card-border bg-card p-4">
        ...
        {invoicesQ.isLoading ? (
          ...
          <EmptyState icon={Clock} title="No overdue invoices" description="All invoices are within their payment terms." />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1000px] text-left text-xs" data-testid="table-overdue-invoices">
              ...
              <tbody>
                {overdueRows.map((r) => (
                  <tr key={r.id} className="border-b border-border/60 bg-destructive/5 last:border-0" data-testid={`row-overdue-${r.id}`}>
