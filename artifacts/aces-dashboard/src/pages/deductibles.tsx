...
export default function DeductiblesPage() {
  ...
  const byProject = useMemo(() => {
    const map = new Map<string, { projectName: string; deductible: number; penalties: number; revenue: number; netRevenue: number }>();
    for (const r of rows) {
  ...
  const totals = useMemo(
    () => byProject.reduce((acc, p) => ({ deductible: acc.deductible + p.deductible, penalties: acc.penalties + p.penalties, revenue: acc.revenue + p.revenue }), { deductible: 0, penalties: 0, revenue: 0 }),
    [byProject],
  );

  const impactData = useMemo(
    () => byProject.map((p) => ({ name: p.projectName, impact: p.deductible + p.penalties, revenue: p.revenue })),
    [byProject],
  );
  ...
  return (
    <AppLayout title="Deductibles & Penalties" description="Breakdown of revenue reductions by project and their impact on collections">
      <FilterBar />
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        ...
        <SummaryStat
          ...
          value={formatPercent(totals.revenue > 0 ? ((totals.deductible + totals.penalties) / totals.revenue) * 100 : 0)}
          accent="chart-1"
        />
      </div>
        ...
        {recordsQ.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px] text-left text-xs" data-testid="table-deductibles-breakdown">
              ...
              <tbody>
                {byProject.map((p) => (
                  <tr key={p.projectName} className="border-b border-border/60 last:border-0" data-testid={`row-deductible-${p.projectName}`}>
...
function SummaryStat({ label, value, accent }: { label: string; value: string; accent: 'destructive' | 'chart-1' }) {
  return (
