...
export default function InvoicesPage() {
    ...
    return (<AppLayout title="Invoice & Collections" description="Full invoice register with payment status and collection tracking">
      <FilterBar showInvoiceStatus/>

      <div className="mb-4 flex items-center gap-2">
        <div className="relative max-w-sm flex-1">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground"/>
          <Input placeholder="Search by invoice no or project..." value={search} onChange={(e) => {
            setSearch(e.target.value);
            setPage(1);
        }} className="pl-9 text-xs" data-testid="input-search-invoices"/>
        </div>
      ...
      {isError ? (<ErrorState onRetry={() => refetch()}/>) : isLoading ? (<TableSkeleton rows={12} cols={10}/>) : rows.length === 0 ? (<EmptyState icon={Receipt} title="No invoices found" description="Try adjusting your search or filters."/>) : (<>
          <div className="overflow-x-auto rounded-lg border border-card-border bg-card">
            <table className="w-full min-w-[1400px] text-left text-xs" data-testid="table-invoices">
              ...
              <tbody>
                {rows.map((r) => (<tr key={r.id} className={`border-b border-border/60 last:border-0 transition-colors ${statusRowClass(r.paymentStatus)}`} data-testid={`row-invoice-${r.id}`}>
          ...
          {data && data.total > PAGE_SIZE && (<div className="mt-4 flex items-center justify-between">
              ...
              <Pagination>
                <PaginationContent>
                  <PaginationItem>
                    <PaginationPrevious onClick={() => setPage((p) => Math.max(1, p - 1))} className={page === 1 ? 'pointer-events-none opacity-50' : 'cursor-pointer'} data-testid="button-page-prev"/>
                  </PaginationItem>
                  <PaginationItem>
                    <PaginationLink className="pointer-events-none">{page} / {totalPages}</PaginationLink>
                  </PaginationItem>
                  <PaginationItem>
                    <PaginationNext onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className={page === totalPages ? 'pointer-events-none opacity-50' : 'cursor-pointer'} data-testid="button-page-next"/>
                  </PaginationItem>
                </PaginationContent>
              </Pagination>
            </div>)}
